...
namespace SFS.Parts.Modules
{
    ...
    {
        [BoxGroup("", false)] public bool attachmentSurfaces = true; // Is the surface used for attachment
        [BoxGroup("", false)] public bool dragSurfaces = true; // Is the surface used for drag
        ...
        {
            Part part = surface.transform.GetComponentInParentTree<Part>();
            Line2[] surfacesWorld = surface.surfaces[0].GetSurfacesWorld();
            
            foreach (PartJoint joint in surface.transform.GetComponentInParentTree<Rocket>().jointsGroup.GetConnectedJoints(part))
                if (SurfaceUtility.SurfacesConnect(joint.GetOtherPart(part), surfacesWorld, out _, out _))
                    return true;
        ...
        // Heat
        [NonSerialized] public HeatModuleBase heatModule;
        protected void Start() => heatModule = transform.GetComponentInParentTree<HeatModuleBase>();
    }
        ...
        public static bool SurfacesConnect(Line2[] surfaces_A, Line2[] surfaces_B, out float overlap, out Vector2 center)
        {
            ...
            foreach (Line2 surface_A in surfaces_A)
                foreach (Line2 surface_B in surfaces_B)
                    if (SurfacesConnect(surface_A, surface_B, out overlap, out center))
                        return true;
