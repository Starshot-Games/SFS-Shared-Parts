...
namespace SFS.Parts.Modules
{
    ...
    public abstract class PolygonData : SurfaceData
    {
        ...
        [SerializeField, BoxGroup("", false), LabelText("Attach By Overlap"), ShowIf(nameof(colliderArea))] bool attachByOverlap = true; // Attach by overlapping colliders
        [SerializeField, BoxGroup("", false), LabelText("Create Physics Collider")] bool physicsCollider = true; // Can the polygon be clicked
        ...
        public virtual bool Raycast(Object debugObject, Vector2 point, out float depth)
        {
            depth = BaseDepth;
            float distance = Polygon.GetDistanceToPolygons(point, polygon.convexPolygons);
            return distance < 0;
        }
    }
}