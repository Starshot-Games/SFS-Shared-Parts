...
namespace SFS.Parts.Modules
{
    ...
    public class PipeMesh : BaseMesh, I_InitializePartModule
    {
        ...
        [FoldoutGroup("Specific")] public bool leftCover, centerCover, rightCover, separatorRing, smoothShading, engineGlowMaterial;
        ...
        // Used by skin module
        [HideInInspector] public Event_Local onSetColorTexture;
    ...
    public class Colors
    {
        ...
        public Color_Channel GetOutput()
        {
            if (mode == Mode.Single)
            {
                ...
                return new Color_Channel(new List<StartEnd_Color>() { new StartEnd_Color(new Color2(c, c), new Line(0, 0)) });
            }
            ...
            {
                ColorKey key = colors[index];
                Color c = key.color.GetColor();
                Color2 _c = new Color2(key.gradient && index > 0? colors[index - 1].color.GetColor() : c, c);
                Line cut = new Line(index == 0 ? 0 : colors[index - 1].height, key.height);
                
                output.elements.Add(new StartEnd_Color(_c, cut));
            }
